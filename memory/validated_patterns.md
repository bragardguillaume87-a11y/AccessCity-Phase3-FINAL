...
## 9. DialogueSmsCard — couleur déterministe par speaker

La fonction `getSmsColors(speakerId, side)` dans `DialogueSmsCard.tsx` utilise un hash stable pour attribuer une couleur unique par speaker. Pattern correct — ne pas "corriger" le hash ou l'index.

```typescript
// ✅ BON — hashIndex() → couleur déterministe (Quilez §14.1)
...
## 10. MinigameBraille — pool de lettres ciblées

`brailleLetters?: string[]` dans `MinigameConfig` (types/game.ts) est intentionnel.
Quand non-vide, `MinigameBraille` tire au sort dans ce sous-ensemble uniquement.

```typescript
// ✅ BON — dans MinigameBraille.tsx
const targetLetterSingle = useMemo(() => {
  const pool = config?.brailleLetters?.length ? config.brailleLetters : BRAILLE_LETTERS;
  return pool[Math.floor(Math.random() * pool.length)];
}, [config?.brailleLetters, roundKey]);
```

---

## 11. uiSounds.minigameTick() — feedback audio dans les formulaires

`uiSounds.minigameTick()` est le son correct pour les odometer arrows (difficulté/durée) dans `MinigameFormPanel`.
Ne pas utiliser `uiSounds.click()` ni implémenter un nouveau son — `minigameTick` est calibré pour ce contexte.

```typescript
// ✅ BON — dans MinigameFormPanel.tsx
onMouseDown={() => {
  setPressedArrow(id);
  setter(val + delta);
...
**Dernière mise à jour :** 2026-04-03 par Claude Sonnet 4.6 (session SMS + Braille + protocole)
