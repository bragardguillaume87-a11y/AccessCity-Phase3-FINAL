...
export function AppProvider({ children }) {
  ...
  const projectSettings = editorState.projectSettings;
  ...
  const [selectedSceneId, setSelectedSceneId] = useState(
    scenes.length ? scenes[0].id : null
  );
  const [selectedSceneForEdit, setSelectedSceneForEdit] = useState(
    scenes.length ? scenes[0].id : null
  );
  ...
  const addScene = useCallback(() => {
    ...
    // Normalise backgroundUrl même si vide
    newScene.backgroundUrl = toAbsoluteAssetPath(newScene.backgroundUrl);
  ...
  const addDialogues = useCallback(
    (sceneId, dialogues) => {
  ...
  const modifyVariable = useCallback(
    (name, delta) => {
      updateEditorState((state) => {
        const current =
          typeof state.variables[name] === "number" ? state.variables[name] : 0;
    ...
    (updates) => {
      updateEditorState((state) => ({
        ...
        projectSettings: {
          ...
          game: { ...state.projectSettings.game, ...(updates.game || {}) },
        },
      }));
    },
    [updateEditorState]
  ...
  const addCharacterToScene = useCallback(
    (sceneId, characterId, mood = "neutral", position = { x: 50, y: 50 }) => {
      updateEditorState((state) => ({
        ...state,
        scenes: state.scenes.map((s) =>
          s.id !== sceneId
