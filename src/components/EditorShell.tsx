...
const MODULE_PLACEHOLDER_LABELS: Record<
  StudioModule,
...
> = {
  ...
  topdown: {
    emoji: '🗺️',
    title: 'Éditeur de carte 2D',
    description:
      'Placez des tuiles, définissez les zones de collision et les triggers de dialogue.',
  },
  behavior: {
    emoji: '🔗',
    title: 'Graphe de comportements',
    description: 'Définissez la logique du jeu en reliant des nodes visuels.',
  },
  'ui-builder': {
    emoji: '🎨',
  ...
  preview: {
    ...
    description: 'Jouez votre jeu topdown avec les dialogues intégrés.',
  },
};
...
function StudioModulePlaceholder({ module }: { module: StudioModule }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-4 bg-background select-none">
      ...
      >
        En développement — Sprint{' '}
        {module === 'topdown' ? 2 : module === 'preview' ? 3 : module === 'behavior' ? 4 : 5}
      </p>
    </div>
  );
}
...
export default function EditorShell({ onBack = null }: EditorShellProps) {
  ...
  useEffect(() => {
    ...
    const oldKeys = ['react-resizable-panels:layout', 'react-resizable-panels:editor-main-group'];
    oldKeys.forEach((key) => {
  ...
  const handleTabChange = (tab: 'scenes' | 'dialogues') => {
    ...
    const targetWidth =
      tab === 'dialogues' ? PANEL_WIDTHS.LEFT_DIALOGUES : PANEL_WIDTHS.LEFT_DEFAULT;
    leftPanelRef.current?.resize(targetWidth);
  };

  // handleOpenModal reste nécessaire pour PropertiesPanel (prop) et le cas graph (spécial)
  const handleOpenModal = useCallback(
    (modal: string, context: unknown = {}) => {
  ...
  const isSceneSelected =
    !selectedElement || selectedElement.type === null || selectedElement.type === 'scene';
  ...
  return (
    <div className="h-screen overflow-hidden bg-background flex flex-col">
      ...
      <KeyboardShortcuts activeTab="editor" setActiveTab={() => {}} />

      <CommandPalette
        isOpen={!!commandPaletteOpen}
        onClose={() => setCommandPaletteOpen(false)}
        mode={
          typeof commandPaletteOpen === 'string'
            ? (commandPaletteOpen as 'commands' | 'quick-open')
            : 'commands'
        }
        setActiveTab={() => {}}
      />
      ...
      <main
        ...
        id="main-content"
        tabIndex={-1}
        style={{ display: activeModule === 'vn-editor' ? undefined : 'none' }}
      >
            ...
            {panel3Width > 0 && (
              ...
              >
                <div className="h-full overflow-hidden" style={{ width: panel3Width }}>
                  <ErrorBoundary name="ContentPanel">{panel3Content}</ErrorBoundary>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            >
              ...
              <Inspector>
                <ErrorBoundary name="UnifiedPanel">
                  <UnifiedPanel onResetLayout={handleResetLayout} />
                </ErrorBoundary>
              </Inspector>
            </div>
          )}
