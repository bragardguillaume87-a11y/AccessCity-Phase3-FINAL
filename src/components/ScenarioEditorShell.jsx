...
function ScenarioEditorShell() {
  ...
  function handleCreateStory(event) {
    ...
    if (demoMode) {
      window.alert(
        "Tu es en mode demo. Les modifications ne seront pas sauvegardees."
      );
      return;
    }
    if (stories.length >= MAX_FREE_STORIES) {
      window.alert(
        "Tu as deja 5 histoires dans cet espace (version gratuite). Supprime une histoire ou passe en version atelier."
      );
      return;
    }
    const newStory = {
  ...
  function handleDeleteStory(storyId) {
    ...
    const confirmMessage =
      "Es tu sur de vouloir supprimer cette histoire ? Cette action est definitive.";
  ...
  function handleDevResetAll() {
    const confirmMessage =
      "ATTENTION DEV : Cela va supprimer TOUTES les histoires et reinitialiser l'onboarding. Confirmer ?";
  ...
  function handleDevClearStories() {
    const confirmed = window.confirm(
      "Supprimer toutes les histoires (onboarding preserve) ?"
    );
    if (!confirmed) return;
    ...
    return (
      ...
      >
        {demoMode && (
          ...
          >
            MODE DEMO - Les modifications ne seront pas sauvegardées
            <button
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <header className="text-center mb-6">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-game-purple to-game-pink text-white rounded-full text-xs font-bold mb-3 shadow-sm">
            ...
            <span className="uppercase tracking-wider">
              AccessCity Studio {demoMode && "· DEMO"}
            </span>
          </div>
          ...
          <p className="text-sm text-slate-600 max-w-2xl mx-auto mb-4">
            Cree des{" "}
            <span className="text-game-purple font-bold">
          ...
          <div className="flex justify-center gap-2 text-xs">
            <span className="px-2 py-1 bg-game-purple/10 border border-game-purple/30 rounded-lg text-game-purple font-semibold">
              🎮 Interface ludique
            </span>
            <span className="px-2 py-1 bg-game-teal/10 border border-game-teal/30 rounded-lg text-game-teal font-semibold">
              ♿ Accessible WCAG AA
            </span>
            <span className="px-2 py-1 bg-game-orange/10 border border-game-orange/30 rounded-lg text-game-orange font-semibold">
              ⚡ Facile à utiliser
            </span>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-6">
          ...
          >
            ...
            <div className="bg-gradient-to-br from-game-purple/5 to-game-teal/5 border border-game-purple/20 rounded-lg p-4 hover:border-game-purple/40 transition-all group">
              <div className="flex items-center justify-between mb-3">
                ...
                <svg
                  className="w-6 h-6 text-game-purple/30 group-hover:text-game-purple/50 transition-colors"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              <div className="flex items-center gap-2 px-2 py-1.5 bg-white rounded-md border border-game-purple/10">
                ...
                <span className="text-xs font-bold text-slate-900">
                  {stories.length}/{MAX_FREE_STORIES} histoires
                </span>
              </div>
            </div>
            ...
            <div className="mt-3 px-3 py-2 bg-game-purple/5 rounded-lg border border-game-purple/10">
              <p className="text-[11px] text-slate-600 leading-relaxed">
                <span className="text-game-purple font-bold">💡</span> Un espace
                regroupe plusieurs histoires. Version gratuite :{" "}
                <span className="font-bold text-game-purple">
                  {MAX_FREE_STORIES} max
                </span>
                .
              </p>
            </div>
          ...
          >
            ...
            {/* Liste des histoires */}
            <div
              className="space-y-3 mb-6 max-h-[400px] overflow-y-auto pr-2"
              role="list"
            >
              {stories.length === 0 && (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    >
                      <svg
                        className={`w-5 h-5 transition-all ${
                          selectedStoryId === story.id
                            ? "text-white"
                            : "text-slate-600 group-hover:text-game-teal"
                        }`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    ...
                    <div className="flex-1 min-w-0">
                      <div
                        className={`text-sm font-bold truncate mb-0.5 transition-colors ${
                          selectedStoryId === story.id
                            ? "text-game-teal"
                            : "text-slate-900"
                        }`}
                      ...
                      <div className="flex items-center gap-1.5 text-[11px]">
                        ...
                        {selectedStoryId === story.id && (
                          <span className="ml-auto text-game-teal font-bold">
                            ✓ Active
                          </span>
                        )}
                      </div>
              ...
              <div className="mb-4 bg-gradient-to-r from-game-orange/5 to-game-pink/5 border border-game-orange/20 rounded-lg p-3">
                ...
                <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${(stories.length / MAX_FREE_STORIES) * 100}%`,
                    }}
                  />
                </div>
                <p
                ...
                >
                  {stories.length >= MAX_FREE_STORIES
                    ? "⚠️ Limite atteinte !"
                    : `✨ ${MAX_FREE_STORIES - stories.length} quête${
                        MAX_FREE_STORIES - stories.length > 1 ? "s" : ""
                      } restante${
                        MAX_FREE_STORIES - stories.length > 1 ? "s" : ""
                      }`}
                </p>
            ...
            <div className="pt-4 border-t border-game-teal/20 space-y-2">
              ...
              <button
                type="button"
                onClick={() =>
                  selectedStory && handleDeleteStory(selectedStory.id)
                }
        ...
        <footer className="mt-12 pt-8 border-t border-slate-200">
          <div className="flex flex-wrap justify-center gap-6 text-xs text-slate-500">
            <button
              type="button"
              className="hover:text-slate-700 underline-offset-2 hover:underline transition-colors"
            >
              Comment ca marche ?
            </button>
            <button
