...
import {
  createDefaultCinematicEvent, getEventDurationMs,
  migrateToCinematicTracks, flattenTracks, getTotalDurationMs,
...
export function CinematicEditor() {
  ...
  useEffect(() => {
    ...
    if (!scene.cinematicTracks) {
      updateCinematicTracks(cinematicEditorSceneId, migrateToCinematicTracks(scene.cinematicEvents ?? []));
    }
    setCurrentTimeMs(0);
    setIsPlaying(false);
  ...
  const previewEvents = useMemo(() => flatEvents.map((te: CinematicTrackedEvent) => te.event), [flatEvents]);
  ...
  useEffect(() => {
    ...
    if (currentTimeMs >= totalMs) { setIsPlaying(false); return; }
  ...
  const handleSelectEvent = useCallback((trackedId: string) => {
  ...
  const handleAddEvent = useCallback((type: CinematicEventType, trackId: CinematicTrackId) => {
    if (!cinematicEditorSceneId) return;
    const newEvent    = createDefaultCinematicEvent(type);
    const trackEvents = tracks[trackId];
    const endMs       = trackEvents.length === 0
      ? 0
      : Math.max(0, ...trackEvents.map((te: CinematicTrackedEvent) => te.startTimeMs + getEventDurationMs(te.event)));
    ...
    updateCinematicTracks(cinematicEditorSceneId, { ...tracks, [trackId]: [...trackEvents, tracked] });
    setSelectedEventId(tracked.id);
    setCurrentTimeMs(tracked.startTimeMs);
    setIsPlaying(false);
  }, [cinematicEditorSceneId, tracks, updateCinematicTracks]);

  const handleDeleteEvent = useCallback((trackedId: string) => {
    if (!cinematicEditorSceneId) return;
    const te = flatEvents.find((t: CinematicTrackedEvent) => t.id === trackedId);
    if (!te) return;
    const newTracks = {
  ...
  const handleUpdateEvent = useCallback((trackedId: string, event: CinematicEvent) => {
    if (!cinematicEditorSceneId) return;
    const te = flatEvents.find((t: CinematicTrackedEvent) => t.id === trackedId);
    if (!te) return;
  ...
  const handleUpdateStartTime = useCallback((trackedId: string, newStartMs: number) => {
    if (!cinematicEditorSceneId) return;
    const te = flatEvents.find((t: CinematicTrackedEvent) => t.id === trackedId);
    if (!te) return;
    updateCinematicTracks(cinematicEditorSceneId, {
