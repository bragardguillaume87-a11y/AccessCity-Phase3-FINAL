...
function accumulateState(events: CinematicEvent[], upToIndex: number, initialBg: string): PreviewState {
  ...
  for (let i = 0; i <= limit; i++) {
    ...
    switch (ev.type) {
      case 'background':
        state.bgUrl = ev.url;
        if (i < upToIndex) { state.dialogue = null; state.titleCard = null; }
        break;
      case 'characterEnter':
...
export function CinematicPreviewCanvas({ events, playheadIndex, backgroundUrl, characterLibrary }: Props) {
  const state = useMemo(
  ...
  const getSidePos = (side: string): React.CSSProperties => {
    switch (side) {
      ...
      case 'top':    return { left: '37%', bottom: '60%' };
      default:       return { left: '37%', bottom: 0 };
    }
  };
  ...
  return (
    ...
    >
      ...
      {state.letterbox && (<>
        <div className="absolute top-0 left-0 right-0 bg-black z-20" style={{ height: '10%' }} />
        <div className="absolute bottom-0 left-0 right-0 bg-black z-20" style={{ height: '10%' }} />
      </>)}
      ...
      {state.titleCard && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/85 z-30 text-center px-4 border border-violet-500/30">
          <span className="text-2xl mb-2 opacity-60" aria-hidden="true">🎞️</span>
          {state.titleCard.title ? (
            <>
              <p className="text-white font-bold drop-shadow-lg" style={{ fontSize: 'clamp(12px, 3vw, 24px)' }}>
                {state.titleCard.title}
              </p>
              {state.titleCard.subtitle && (
                <p className="text-white/70 mt-1 drop-shadow" style={{ fontSize: 'clamp(10px, 2vw, 16px)' }}>
                  {state.titleCard.subtitle}
                </p>
              )}
            </>
        ...
        <div className="absolute bottom-0 left-0 right-0 z-50 bg-black/80 px-3 py-2 border-t border-white/10">
          {state.dialogue.speaker && (
            <p className="text-violet-300 text-[10px] font-bold mb-0.5 truncate">
              ...
              {characterLibrary.find(c => c.id === state.dialogue!.speaker)?.name ?? state.dialogue.speaker}
            </p>
          )}
          <p className="text-white leading-snug line-clamp-3" style={{ fontSize: 'clamp(9px, 1.5vw, 13px)' }}>
        ...
        <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-50 bg-black/60 backdrop-blur-sm px-2 py-0.5 rounded-full text-white/70 text-[10px] pointer-events-none">
          {(() => {
            ...
            const labels: Partial<Record<string, string>> = {
              fade: '🌑 Fondu', flash: '⚡ Flash', screenShake: '📳 Tremblement',
              wait: '⏱️ Pause', sfx: '🔊 Effet sonore', bgm: '🎵 Musique de fond',
