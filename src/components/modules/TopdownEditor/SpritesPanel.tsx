...
export default function SpritesPanel({
  ...
  const filteredAssets = spriteAssets.filter(a => {
    ...
    const searchMatch = !searchQuery || displayName.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const selectedAsset = spriteAssets.find(a => (a.url ?? a.path) === selectedSpriteUrl || a.path === selectedSpriteUrl);
  ...
  const openImport = useCallback((a: typeof spriteAssets[0]) => {
    const url = a.url ?? a.path;
  ...
  const handleImportConfirm = useCallback((imagePath: string, config: SpriteSheetConfig) => {
    ...
    return (
      ...
      >
        ...
        <div style={{
          ...
          border: `1px solid ${isSelected ? 'rgba(139,92,246,0.5)' : 'var(--color-border-base)'}`,
        ...
        <div style={{ flex: 1, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            {isPlayer && (
              <span style={{ fontSize: 9, padding: '1px 4px', borderRadius: 3, background: 'rgba(139,92,246,0.25)', color: 'var(--color-primary)', fontWeight: 700, flexShrink: 0 }}>
                🎮
              </span>
            )}
          </div>
          {getConfig(a) && (
            <span style={{ fontSize: 10, color: 'var(--color-text-muted)' }}>
              {getConfig(a)!.frameW}×{getConfig(a)!.frameH}px · {Object.keys(getConfig(a)!.animations).length} anim.
            </span>
          )}
        </div>
        ...
        {/* Configure button */}
        <button
          onClick={e => { e.stopPropagation(); openImport(a); }}
        ...
        <button
          onClick={e => { e.stopPropagation(); hideAsset(a.url ?? a.path); }}
  ...
  const renderAnimsSummary = (config: SpriteSheetConfig) => {
    ...
    if (configured.length === 0) {
      return (
        <p style={{ margin: 0, fontSize: 11, color: 'var(--color-text-muted)', fontStyle: 'italic' }}>
    ...
    >
      ...
      <div style={{ borderBottom: '1px solid var(--color-border-base)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 5, alignItems: 'center', marginBottom: 5 }}>
          {/* Search */}
          <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(0,0,0,0.3)', border: '1px solid var(--color-border-base)', borderRadius: 5, padding: '3px 7px' }}>
            ...
            <input
              ...
              style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: 'var(--color-text-base)', fontSize: 12 }}
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', padding: 0, display: 'flex' }}>
                <X size={10} />
              </button>
            )}
          </div>
          ...
          <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
              <button title="Filtrer par catégorie" style={{
                display: 'flex', alignItems: 'center', gap: 3, padding: '4px 7px', borderRadius: 5, cursor: 'pointer',
                border: filterCategory ? '1.5px solid rgba(139,92,246,0.6)' : '1px solid var(--color-border-base)',
                background: filterCategory ? 'rgba(139,92,246,0.15)' : 'rgba(0,0,0,0.2)',
                color: filterCategory ? 'var(--color-primary)' : 'var(--color-text-muted)',
                fontSize: 13, fontWeight: filterCategory ? 700 : 400, flexShrink: 0,
              }}>
            ...
            <DropdownMenu.Portal>
              <DropdownMenu.Content side="bottom" align="end" sideOffset={4} style={{
                ...
                <DropdownMenu.Item onSelect={() => setFilterCategory(null)} style={{
                  display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', cursor: 'pointer',
                  background: !filterCategory ? 'rgba(139,92,246,0.15)' : 'transparent',
                  borderBottom: '1px solid var(--color-border-base)', outline: 'none',
                  color: !filterCategory ? 'var(--color-primary)' : 'var(--color-text-base)',
                  fontSize: 13, fontWeight: !filterCategory ? 700 : 400,
                }}>
                  <span style={{ fontSize: 16 }}>🎭</span> Toutes
                  <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--color-text-muted)', background: 'rgba(255,255,255,0.08)', padding: '1px 6px', borderRadius: 8 }}>
                    {spriteAssets.length}
                  </span>
                </DropdownMenu.Item>
                {SPRITE_CATEGORIES.map(cat => {
                  ...
                  return (
                    <DropdownMenu.Item key={cat.id} onSelect={() => setFilterCategory(cat.id)} style={{
                      display: 'flex', alignItems: 'center', gap: 8, padding: '7px 12px', cursor: 'pointer',
                      background: isActive ? 'rgba(139,92,246,0.15)' : 'transparent',
                      outline: 'none', fontSize: 13, fontWeight: isActive ? 700 : 400,
                      color: isActive ? 'var(--color-primary)' : 'var(--color-text-base)',
                    }}>
                      <span style={{ fontSize: 16 }}>{cat.emoji}</span>
                      <span style={{ flex: 1 }}>{cat.label}</span>
                      <span style={{ fontSize: 11, color: 'var(--color-text-muted)', background: 'rgba(255,255,255,0.08)', padding: '1px 6px', borderRadius: 8 }}>
                        {count}
                      </span>
                    </DropdownMenu.Item>
                  );
          ...
          <label
            ...
            style={{
              ...
              border: '1px solid var(--color-border-base)',
              background: isUploading ? 'rgba(139,92,246,0.2)' : 'rgba(0,0,0,0.2)',
        ...
        {filterCategory && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Filtre :</span>
            <span style={{
              display: 'flex', alignItems: 'center', gap: 4,
              fontSize: 11, fontWeight: 600, color: 'var(--color-primary)',
              background: 'rgba(139,92,246,0.15)', padding: '1px 7px', borderRadius: 10,
            }}>
              {filterCatObj?.emoji} {filterCatObj?.label}
              <button
                onClick={() => setFilterCategory(null)}
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-primary)', padding: 0, display: 'flex', marginLeft: 2 }}
              >
                <X size={10} />
              </button>
            </span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 4px' }}>
        {loading && spriteAssets.length === 0 ? (
          ...
          <div style={{ padding: '6px 4px', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {[60, 75, 50, 68, 55].map((w, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 8px' }}>
                <div style={{ width: 36, height: 36, borderRadius: 4, background: 'rgba(255,255,255,0.08)', flexShrink: 0, animation: `shimmer 1.4s ease-in-out ${i * 0.12}s infinite` }} />
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 5 }}>
                  <div style={{ height: 11, borderRadius: 4, background: 'rgba(255,255,255,0.08)', width: `${w}%`, animation: `shimmer 1.4s ease-in-out ${i * 0.12}s infinite` }} />
                  <div style={{ height: 9, borderRadius: 4, background: 'rgba(255,255,255,0.05)', width: '45%', animation: `shimmer 1.4s ease-in-out ${i * 0.12 + 0.05}s infinite` }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ padding: '24px 12px', textAlign: 'center' }}>
            <p style={{ margin: 0, fontSize: 13, color: 'var(--color-text-muted)' }}>Aucun sprite importé</p>
            <p style={{ margin: '6px 0 0', fontSize: 11, color: 'var(--color-text-muted)', opacity: 0.7 }}>
              Cliquez sur ⬆ ou glissez une image PNG/spritesheet
            </p>
          </div>
        ) : filteredAssets.length === 0 ? (
          <div style={{ padding: '16px 12px', textAlign: 'center' }}>
            <p style={{ margin: 0, fontSize: 12, color: 'var(--color-text-muted)' }}>Aucun résultat</p>
          </div>
              ...
              return (
                <div key={cat.id}>
                  <div style={{
                    ...
                    fontSize: 11, color: 'var(--color-text-muted)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em',
                  }}>
                    <span style={{ fontSize: 13 }}>{cat.emoji}</span>
                    {cat.label}
                    <span style={{ marginLeft: 'auto', fontSize: 10, background: 'rgba(255,255,255,0.1)', padding: '1px 5px', borderRadius: 8 }}>
                      {catAssets.length}
                    </span>
                  </div>
      ...
      {selectedAsset && (
        <div style={{ borderTop: '1px solid var(--color-border-base)', padding: '10px 10px', flexShrink: 0 }}>
          {/* Sprite header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <img
              src={selectedAsset.url ?? selectedAsset.path}
              alt={getDisplayName(selectedAsset)}
              style={{ width: 40, height: 40, objectFit: 'contain', imageRendering: 'pixelated', border: '1px solid var(--color-border-base)', borderRadius: 5, background: 'rgba(0,0,0,0.4)' }}
            />
            <div style={{ flex: 1, overflow: 'hidden' }}>
              <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: 'var(--color-text-base)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {getDisplayName(selectedAsset)}
              </p>
              {selectedConfig && (
                <p style={{ margin: 0, fontSize: 10, color: 'var(--color-text-muted)' }}>
                  {selectedConfig.frameW}×{selectedConfig.frameH}px · {selectedConfig.cols}×{selectedConfig.rows} grille
                </p>
              )}
            </div>
            <button
              ...
              title="Configurer le spritesheet"
              style={{
                display: 'flex', alignItems: 'center', gap: 4, padding: '4px 8px', borderRadius: 5, cursor: 'pointer',
                border: '1px solid var(--color-border-base)',
          ...
          {selectedConfig ? (
            ...
            <div style={{ marginBottom: 10, padding: '6px 8px', borderRadius: 5, background: 'rgba(139,92,246,0.08)', border: '1px dashed rgba(139,92,246,0.3)' }}>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                <button
                  onClick={() => {
                    ...
                    const isPlayer = url === playerSpritePath || selectedAsset.path === playerSpritePath;
                    updateMapMetadata(mapId, { playerSpritePath: isPlayer ? undefined : url });
                  }}
                  style={{
                    flex: 1, padding: '5px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, cursor: 'pointer',
                    border: (selectedAsset.url ?? selectedAsset.path) === playerSpritePath
                      ? '1.5px solid rgba(139,92,246,0.8)'
                      : '1px solid var(--color-border-base)',
                    background: (selectedAsset.url ?? selectedAsset.path) === playerSpritePath
                      ? 'rgba(139,92,246,0.2)'
                      : 'transparent',
                    color: (selectedAsset.url ?? selectedAsset.path) === playerSpritePath
                      ? 'var(--color-primary)'
            ...
            <div>
                ...
                {ENTITY_BEHAVIORS.map(b => (
                  <button
                    ...
                    style={{
                      ...
                      border: entityBehavior === b.id ? '1.5px solid rgba(139,92,246,0.7)' : '1px solid var(--color-border-base)',
                      background: entityBehavior === b.id ? 'rgba(139,92,246,0.15)' : 'transparent',
                      color: entityBehavior === b.id ? 'var(--color-primary)' : 'var(--color-text-muted)',
                    }}
                  >
                    <span style={{ fontSize: 14 }}>{b.emoji}</span>
                    <span style={{ fontSize: 9, fontWeight: entityBehavior === b.id ? 700 : 400 }}>{b.label}</span>
                  </button>
                ))}
              </div>
              ...
              {isPlacingEntity ? (
                ...
                <button
                  ...
                  style={{
                    width: '100%', padding: '7px', borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: 'pointer',
                ...
                <button
                  ...
                  style={{
                    ...
                    background: 'rgba(139,92,246,0.18)', color: 'var(--color-primary)',
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5,
                  }}
                >
