...
export default function TilesetImportDialog({
  ...
  const [displayName, setDisplayName] = useState<string>(initialConfig?.displayName ?? imageName ?? '');
  ...
  const handleConfirm = () => {
    onConfirm(imagePath, {
      tileW: eTileW, tileH: eTileH, margin, spacing,
      category,
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <Dialog.Title style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--color-text-base)' }}>
                Configurer le tileset
              </Dialog.Title>
              {imageName && (
                <Dialog.Description style={{ margin: 0, fontSize: 11, color: 'var(--color-text-muted)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {imageName}
                </Dialog.Description>
              )}
            </div>
            {imgSize && (
              <span style={{ fontSize: 12, color: 'var(--color-text-muted)', flexShrink: 0, background: 'rgba(255,255,255,0.06)', padding: '3px 8px', borderRadius: 4 }}>
            ...
            <div style={{ flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                {TILESET_CATEGORIES.map(cat => {
                  ...
                  return (
                    <button key={cat.id} onClick={() => setCategory(cat.id)} title={cat.label}
                      style={{
                        display: 'flex', alignItems: 'center', gap: 4,
                        padding: '5px 9px', borderRadius: 6, cursor: 'pointer',
                        border: active ? '1.5px solid var(--color-primary)' : '1px solid var(--color-border-base)',
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
              {PRESETS.map(p => {
                ...
                return (
                  <button key={p.label} onClick={() => { setTileW(p.w); setTileH(p.h); }}
                    style={{
                      padding: '5px 12px', borderRadius: 6, fontSize: 13, fontWeight: active ? 700 : 500,
                      border: active ? '1.5px solid var(--color-primary)' : '1.5px solid var(--color-border-base)',
                      background: active ? 'rgba(139,92,246,0.18)' : 'transparent',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <input type="number" value={tileW} min={1} max={512}
                  onChange={e => setTileW(Math.max(1, parseInt(e.target.value) || 1))}
                  style={numInputStyle} title="Largeur tuile (px)" />
                <span style={{ color: 'var(--color-text-muted)', fontSize: 13 }}>×</span>
                <input type="number" value={tileH} min={1} max={512}
                  onChange={e => setTileH(Math.max(1, parseInt(e.target.value) || 1))}
                  style={numInputStyle} title="Hauteur tuile (px)" />
                <span style={{ color: 'var(--color-text-muted)', fontSize: 12 }}>px</span>
              </div>
              {totalTiles > 0 && (
              ...
              <div style={{ display: 'flex', gap: 20, marginTop: 10 }}>
                <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <input type="number" value={margin} min={0} max={64}
                ...
                <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <input type="number" value={spacing} min={0} max={64}
          ...
          <div>
              ...
              {({ zoomIn, zoomOut, resetTransform }) => (
                <>
                  <div style={{ display: 'flex', gap: 4, marginBottom: 5, justifyContent: 'flex-end' }}>
                    <button onClick={() => zoomOut()} style={previewBtn} title="Zoom arrière"><ZoomOut size={12} /></button>
                    <button onClick={() => resetTransform()} style={previewBtn} title="Réinitialiser"><Maximize2 size={12} /></button>
                    <button onClick={() => zoomIn()} style={previewBtn} title="Zoom avant"><ZoomIn size={12} /></button>
                  </div>
                  <div style={{
                    ...
                    >
                      ...
                      {cols > 0 && rows > 0 && imgSize && (
                        <div style={{ position: 'absolute', top: margin, left: margin, pointerEvents: 'none' }}>
            ...
            {imgSize && totalTiles === 0 && (
              <p style={{ margin: '6px 0 0', fontSize: 12, color: 'var(--color-text-warning, #f59e0b)' }}>
          ...
          {/* Actions */}
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', paddingTop: 4, borderTop: '1px solid var(--color-border-base)' }}>
...
const textInputStyle: React.CSSProperties = {
  width: '100%', padding: '6px 10px', borderRadius: 5, boxSizing: 'border-box',
  border: '1px solid var(--color-border-base)',
  background: 'rgba(0,0,0,0.3)', color: 'var(--color-text-base)', fontSize: 13,
};
const numInputStyle: React.CSSProperties = {
  width: 60, padding: '5px 6px', borderRadius: 4, textAlign: 'center',
  border: '1px solid var(--color-border-base)',
  background: 'rgba(0,0,0,0.3)', color: 'var(--color-text-base)', fontSize: 13,
};
