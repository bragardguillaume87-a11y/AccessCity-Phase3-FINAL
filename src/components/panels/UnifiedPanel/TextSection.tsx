...
function SliderRow({ label, value, min, max, step, unit, onChange, ariaLabel }: {
  ...
  step: number; unit: string; onChange: (v: number) => void; ariaLabel: string;
}) {
  return (
    <div className="mb-3">
      ...
      <input
        ...
        className="sp-slider" aria-label={ariaLabel}
      />
    </div>
  );
}
...
function ToggleGroup<T extends string>({ label, value, options, onChange }: {
  ...
  return (
    <div className="mb-3">
      <p className="text-[11px] font-medium text-[var(--color-text-muted)] uppercase tracking-wide mb-2">{label}</p>
  ...
  return (
    <div>
      ...
      <section className="sp-sec" aria-labelledby="dlgbox-text-heading">
        ...
        <SliderRow
          label="Vitesse de frappe" value={cfg.typewriterSpeed}
        ...
        />
        <SliderRow
          label="Taille du texte" value={cfg.fontSize}
        ...
        {cfg.showPortrait && (
          <div className="space-y-1 pl-2 border-l-2 border-[var(--color-primary)]/30 mb-3">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium text-[var(--color-text-secondary)]">Cadrage portrait</p>
              <button
