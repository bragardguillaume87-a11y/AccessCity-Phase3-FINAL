 ...
 *
 * @param {string} key - localStorage key (use STORAGE_KEYS constants)
 * @param {*} initialValue - Initial value if key doesn't exist
 * @returns {[value, setValue, removeValue]} State value, setter function, and remove function
 *
 * @example
 * import { STORAGE_KEYS } from '@/config/storageKeys';
 ...
 * @returns {[history, addToHistory, clearHistory]} History array, add function, clear function
 *
 * @example
 * const [bgHistory, addBg] = useLocalStorageHistory(STORAGE_KEYS.BACKGROUNDS_HISTORY, 10);
 * addBg('/assets/backgrounds/city.jpg');
 */
export function useLocalStorageHistory(key, maxItems = 10) {
  const [history, setHistory, clearHistory] = useLocalStorage(key, []);
 ...
 * @returns {[value, setTrue, setFalse, toggle]} Value, setTrue, setFalse, toggle functions
 *
 * @example
 ...
 */
export function useLocalStorageFlag(key, initialValue = false) {
 ...
 * @param {Function} callback - Callback function when value changes in another tab
 *
 * @example
 ...
 * });
 */
export function useLocalStorageSync(key, callback) {
  useEffect(() => {
 ...
 *
 * @param {string} prefix - Prefix to filter keys (e.g., 'ac_')
